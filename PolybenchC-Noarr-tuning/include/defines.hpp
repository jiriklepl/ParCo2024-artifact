#ifndef NOARR_POLYBENCH_TUNING_DEFINES_HPP
#define NOARR_POLYBENCH_TUNING_DEFINES_HPP
...
#define MAKE_OPENTUNER_FORMATTER(algorithm) \
    noarr::tuning::opentuner_formatter( \
		std::cout, \
		noarr::tuning::cmake_compile_command_builder("..", ".", #algorithm), \
		noarr::tuning::direct_run_command_builder("./" #algorithm), \
		"return Result(time=float(run_result['stderr'].split()[0]))")

#define MAKE_OPTUNA_FORMATTER(algorithm) \
	noarr::tuning::optuna_formatter( \
		std::cout, \
		noarr::tuning::cmake_compile_command_builder("..", ".", #algorithm), \
		noarr::tuning::direct_run_command_builder("./" #algorithm), \
		"return float(run_result.stderr.decode().split()[0])")

#endif // NOARR_POLYBENCH_TUNING_DEFINES_HPP
