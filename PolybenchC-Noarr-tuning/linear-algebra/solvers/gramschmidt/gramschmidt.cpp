...
struct tuning {
	NOARR_TUNE_BEGIN(MAKE_OPENTUNER_FORMATTER("gramschmidt"));

	NOARR_TUNE_PAR(a_layout, noarr::tuning::choice, k_vec ^ i_vec, i_vec ^ k_vec);
	NOARR_TUNE_PAR(r_layout, noarr::tuning::choice, j_vec ^ k_vec, k_vec ^ j_vec);
	NOARR_TUNE_PAR(q_layout, noarr::tuning::choice, k_vec ^ i_vec, i_vec ^ k_vec);
